...
PASCAL_VOC_ALL_CLASSES = './data/pascal_voc07_cls_names.txt'
...
def main():
	...
	trainset = Dataset('train', NEW_CLASSES_TO_LEARN, MODEL_OBJECTS_AFTER_INCRE)  # in this place New classes to learn - car & person , MOdel objects after incre car + person u can give another name. 
	...
	yolo_prev = Create_Yolov3(input_size = YOLO_INPUT_SIZE, training = False, CLASSES =  './data/coco.names', dot_name_file= True) # here this model will take coco weights 
	yolo_new = Create_Yolov3(input_size=YOLO_INPUT_SIZE, training= True, 
		CLASSES=MODEL_OBJECTS_AFTER_INCRE, dot_name_file = False) # in this txt file u have car person yes



	# yolo_prev.load_weights('./checkpoints/yolov3_coco.ckpt')

	load_yolo_weights(yolo_prev, './checkpoints/yolov3.weights')


	# load the weights for the previous checkpont 
	# yolo_new.load_weig/hts('./checkpoints/yolov3_custom_val_loss_  23.96')
	# load_yolo_weights(yolo_prev, weights_file)
	# yolo_new.load_weights('./checkpoints/yolov3_custom_val_loss_   4.14')
